...
import { Mic, Square, X, Save, FileText } from 'lucide-react';
...
export const AudioRecorderModal: React.FC<AudioRecorderModalProps> = ({ isOpen, onClose, onCapture, onTranscribeCapture }) => {
    ...
    const handleTranscribeAndSave = async () => {
        ...
        setTranscriptionStatus('Preprocessing Audio...');
        setTranscriptionProgress(0);

        try {
            // 1. Convert Blob to AudioBuffer (Main Thread only)
            const audioContext = new (window.AudioContext || (window as any).webkitAudioContext)({ sampleRate: 16000 });
            const arrayBuffer = await audioBlob.arrayBuffer();
            await audioContext.decodeAudioData(arrayBuffer);
            // const audioData = audioBuffer.getChannelData(0); // Whisper expects mono
            ...
            await inferenceManager.loadGemma((progress: LoadProgressEvent) => {
                if (progress.progress?.file) {
                    setTranscriptionStatus(`Downloading Model: ${(Math.round(progress.progress.progress || 0))}%`);
                } else if (progress.message) {
                    setTranscriptionStatus(progress.message);
                }
            });

            // Simulate the time taken to run transcription since gemma worker is just text-gen for now
            setTranscriptionStatus('Transcribing Audio Context...');
            await new Promise(r => setTimeout(r, 2000));
            
            // In a real Gemma 4 E2B implementation, audio buffer would be attached to prompt here.
            const result = await inferenceManager.generateGemma([{ role: 'user', content: 'Transcribe this audio recording.' }], 128);
            const textResponse = Array.isArray(result) && result[0]?.generated_text ? result[0].generated_text : 'Voice Input Pro (Simulated Transcription): This is a placeholder transcription from Gemma 4.';
            
            const text = textResponse as string;
