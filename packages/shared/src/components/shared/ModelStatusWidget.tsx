...
interface ModelStatusWidgetProps {
    fetchStatus: () => Promise<SharedModelStatus> | SharedModelStatus;
}

export const ModelStatusWidget = ({ fetchStatus }: ModelStatusWidgetProps) => {
    const [status, setStatus] = useState<SharedModelStatus | null>(null);
    ...
    const loadingCount = status?.loadingModels?.length || 0;
    const totalMem =
        status?.loadedModels.reduce((acc, model) => acc + (model.memoryFootprintMB || 0), 0) || 0;
    ...
    return (
        <div className="relative">
            ...
            >
                <Server className={`w-4 h-4 ${loadingCount > 0 ? 'animate-pulse' : ''}`} />
                <span className="text-xs font-bold uppercase tracking-wider hidden md:inline">
                    {loadedCount + loadingCount} Models ({totalMem}MB)
                </span>
            </button>
                    ...
                    <div className="p-4 space-y-4 max-h-96 overflow-y-auto">
                        {loadedCount === 0 && loadingCount === 0 ? (
                            ...
                            <>
                                ...
                                {status?.loadingModels?.map((model) => (
                                    <div key={model.id} className="bg-amber-50 border border-amber-200 p-3 shadow-sm relative overflow-hidden animate-pulse">
                                        <div className="absolute top-0 left-0 w-1 h-full bg-amber-400" />
                                        <div className="pl-3">
                                            <div className="flex justify-between items-start mb-1">
                                                <span className="text-sm font-bold text-gray-700 pr-2">{model.name}</span>
                                                <span className="text-[9px] font-mono text-amber-600 bg-amber-100 px-1.5 py-0.5 border border-amber-200 mt-0.5">Loading…</span>
                                            </div>
                                            <div className="flex justify-between items-center mt-3 pt-2 border-t border-amber-100">
                                                <span className="text-[10px] text-gray-500 uppercase tracking-wide">Memory Footprint</span>
                                                <span className="text-xs font-black text-amber-600">{model.memoryFootprintMB} MB</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="mt-4 pt-3 border-t border-gray-100 flex items-start gap-2 text-gray-400">
                            ...
                            <p className="text-[9px] uppercase tracking-wider leading-relaxed">
                                The active inference manager automatically evicts older models when memory pressure rises.
                            </p>
                        </div>
                    </div>
                </div>
