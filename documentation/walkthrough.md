...
The project has undergone a significant architectural shift from **TensorFlow.js (Universal Sentence Encoder)** to **Transformers.js (@xenova/transformers)** for its primary natural language processing and multimodal tasks.
...
I have updated both the [technical_specification.md](file:///Users/tblanke/Downloads/public-ai-app/difference-suite/documentation/technical_specification.md) and [deep_culture_article.md](file:///Users/tblanke/Downloads/public-ai-app/difference-suite/documentation/deep_culture_article.md) with the following specific details:

| Component | Technology / Model | New Features Documented |
| :--- | :--- | :--- |
| **Embeddings** | `all-MiniLM-L6-v2` | Used across 7+ tools for semantic mapping and search. |
...
| **Multimodal** | `CLIP` | "Visual Synapse" for cross-modal alignment in Networked Narratives. |
| **Audio/ASR** | `Whisper Tiny` | Real-time voice transcription for dataset capture. |
| **Latent Analysis** | **Attention Lens** | Visualizing internal model attention in Deep Vector Mirror. |

## Documentation Integrity

- **All 13 Tools**: accounted for in both the technical specification and the journal article.
- **GitHub Consistency**: Verified against the `main` branch source code for individual components like `GlitchDetectorText`, `AmbiguityAmplifierText`, and `ContextWeaver`.
